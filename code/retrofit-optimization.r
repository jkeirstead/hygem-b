...
## If this is TRUE, it will use pre-stored values for comparison
## with Mark's GAMS model
debug <- TRUE
...
annuity_factor <- g/(1 - ((1+g)**(-n)))

## Fuel cost
## Output from AREVA project by Shah et al. (2013)
fuel_costs <- data.frame(region="OECD Europe", LMS=13.65, LCS=16.05)
fuel.m <- melt(fuel_costs, id="region", variable.name="scenario", value.name="costs")

tmp <- merge(fuel.m, resi)
tmp <- transform(tmp, amount=costs*demand)
costs <- dcast(tmp, region ~ scenario, value.var="amount")
costs <- transform(costs, saving=LCS - LMS)
print(costs)

## Calculate the total cost
tmp <- ldply(models, function(l) data.frame(cost=l$objval))
tmp <- merge(regional_factors, tmp)
tmp <- transform(tmp, AF=annuity_factor)
tmp <- merge(tmp, costs, all.x=TRUE)
tmp <- transform(tmp, LMS=ifelse(is.na(LMS), 0, LMS),
                 LCS=ifelse(is.na(LCS), 0, LCS),
                 saving=ifelse(is.na(saving), 0, saving))

tmp2 <- transform(tmp, total_cost=operating*(saving) + capital*cost*AF)

tmp3 <- merge(tmp2, regions)
hhcosts <- transform(tmp3, hh_cost=total_cost/nhouses, total_cost=total_cost/1e9)
hhcosts <- hhcosts[,c("region", "total_cost", "hh_cost")]

print(hhcosts)

message("These results look fine except for the weird OECD Europe correction.  Ask Mark")
